...
log_mean_T = lmtd('counter-current', high_acetone_T, low_acetone_T, high_water_T, low_water_T)
diameter ,ext_diam, int_diam, Flow_area, lin_surface = config_diameters('4*3')

equivalent_diam, annulus_area, pipe_area = flow_area(ext_diam, int_diam)
...
corrected_pipe_conv_coeff = corrected_inside_coeff(pipe_convective_coeff, (diameter, int_diam))
...
corrected_annulus_Reynold = recalculate_reynolds(ext_diam, int_diam, water_mass_flow, water_viscosity)
annulus_friction_factor = friction_factor(corrected_annulus_Reynold)
annulus_Fanning_factor = Fanning_factor(annulus_friction_factor, water_mass_flow, new_length, water_density, (ext_diam - int_diam))
...
print(f'The real heat trasnfer is: {(effectivity*Q_max):.3f} Btu/h')
print(f'The max heat transfer is: {Q_max:.3f} Btu/h')
print(f'The exchanger efectivity is: {(effectivity * 100):.3f}%')